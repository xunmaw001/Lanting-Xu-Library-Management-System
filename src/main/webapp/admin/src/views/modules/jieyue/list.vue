                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('jieyue','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/tushuguanlixitongjiangxi/upload/jieyueMuBan.xls"
                        >批量导入借阅记录数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('jieyue','导入导出')"
                                ...
                                :on-success="jieyueUploadSuccess"
                                :on-error="jieyueUploadError"
                        ...
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('jieyue','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "jieyue.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                                     ...
                                     label="操作">
                        <template slot-scope="scope">
                            <el-button v-if="isAuth('jieyue','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('jieyue','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('jieyue','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
                            <el-button v-if="isAuth('jieyue','还书') && scope.row.jieyueTypes != 2 && scope.row.jieyueTypes != 4" type="primary" icon="el-icon-edit" size="mini" @click="huanshu(scope.row.id,scope.row.yonghuId,scope.row.tushuId,scope.row.jieyueTypes)">还书</el-button>
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :"jieyue",//当前表表名,
                        sumColum : 'jieyue_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'jieyue_name',//分组字符串字段
                        // types : 'jieyue_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                params['jieyueDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
			...
			huanshu(id,yonghuId,tushuId,jieyueTypes){
                let data = null;
                if(jieyueTypes==1){
                    data = {
                        id:id,
            ...
            // 导入功能上传文件成功后调用导入方法
            jieyueUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "jieyue/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
