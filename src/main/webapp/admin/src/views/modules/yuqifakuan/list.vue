        ...
        <div v-if="showFlag">
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('yuqifakuan','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/tushuguanlixitongjiangxi/upload/yuqifakuanMuBan.xls"
                        >批量导入逾期罚款数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('yuqifakuan','导入导出')"
                                ...
                                :on-success="yuqifakuanUploadSuccess"
                                :on-error="yuqifakuanUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('yuqifakuan','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入逾期罚款数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('yuqifakuan','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "yuqifakuan.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="yuqifakuanNumber"
                                   header-align="center"
                                   label="罚款金额">
                        <template slot-scope="scope">
                            {{scope.row.yuqifakuanNumber}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="qinyoumokuaiText"
                                      header-align="center"
                                      label="罚款原因">
                        <template slot-scope="scope">
                            <span v-if="scope.row.qinyoumokuaiText != null &&scope.row.qinyoumokuaiText.length>10">
                                {{scope.row.qinyoumokuaiText.slice(0,10)}}...
                            </span>
                            <span v-else>
                                {{scope.row.qinyoumokuaiText}}
                            </span>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                                     ...
                                     label="操作">
                        <template slot-scope="scope">
                            <el-button v-if="isAuth('yuqifakuan','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('yuqifakuan','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>

                            <el-button v-if="isAuth('yuqifakuan','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
            form:{
                id : null,
                yonghuId : null,
                yuqifakuanNumber : null,
                qinyoumokuaiText : null,
                insertTime : null,
                createTime : null,
            },
            dataList: [],
            ...
            json_fields: {
                ...
                //本表字段
                     '罚款金额': "yuqifakuanNumber",
                     '罚款原因': "qinyoumokuaiText",
                     '添加时间': "insertTime",
            },
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'yuqifakuan',//当前表表名,
                        sumColum : 'yuqifakuan_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'yuqifakuan_name',//分组字符串字段
                        // types : 'yuqifakuan_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                params['yuqifakuanDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
            ...
            // 导入功能上传文件成功后调用导入方法
            yuqifakuanUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "yuqifakuan/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
