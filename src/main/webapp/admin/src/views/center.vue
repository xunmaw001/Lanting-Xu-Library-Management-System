  ...
  <div>
    ...
    >  
     <el-row>
		...
		<el-col>
           <el-form-item v-if="flag=='yonghu'"  label='可借阅数量' prop="jieyueshuliang">
               <el-input v-model="ruleForm.jieyueshuliang"  placeholder='可借阅数量' readonly clearable></el-input>
           </el-form-item>
         </el-col>
...
export default {
  ...
  mounted() {
    ...
    this.$http({
      ...
      if (data && data.code === 0) {
		...
		if (this.role != "管理员"){
			this.$http({
			  url: `jieyue/page?jieyueTypes=3`,
			  method: "get",
			}).then(({ data }) => {
			  if (data.data.list.length>0 && data.code === 0) {
				this.$message.error('您有借阅的图书已经超时,请及时还书');
			  }
			});
		}
  ...
  methods: {
    ...
    onUpdateHandler() {
                         ...
                         if((!this.ruleForm.jieyueshuliang)&& 'yonghu'==this.flag){
                             this.$message.error('可借阅数量不能为空');
                             return
                         }
